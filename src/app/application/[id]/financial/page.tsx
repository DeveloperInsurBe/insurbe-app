...
function validateStep(stepIndex: number, form: Form): string | null {
  switch (stepIndex) {
    case 0: if (!form.employmentStatus) return "Please select your employment status."; return null;
    case 1: if (!form.jobTitle?.trim()) return "Job title is required."; return null;
    case 2: if (!form.employerName?.trim()) return "Employer name is required."; return null;
    case 3:
      if (!form.startDay || !form.startMonth || !form.startYear) return "Please enter your employment start date.";
      if (isNaN(Number(form.startDay)) || Number(form.startDay) < 1 || Number(form.startDay) > 31) return "Invalid day.";
      if (isNaN(Number(form.startMonth)) || Number(form.startMonth) < 1 || Number(form.startMonth) > 12) return "Invalid month.";
      if (isNaN(Number(form.startYear)) || Number(form.startYear) < 1950 || Number(form.startYear) > new Date().getFullYear() + 5) return "Invalid year.";
      return null;
    case 4: return null; // work contract — optional (provide later)
    case 5:
      if (!form.annualIncome?.toString().trim()) return "Please enter your annual income.";
      if (isNaN(Number(form.annualIncome)) || Number(form.annualIncome) < 0) return "Enter a valid income.";
      return null;
    case 6: if (!form.employedOutsideGermany) return "Please select an option."; return null;
    case 7: if (!form.hasGermanTaxId) return "Please select an option."; return null;
    case 8: if (!form.taxIdAccepted) return "Please accept the terms to continue."; return null;
    default: return null;
  }
}
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm text-slate-900 font-medium truncate">
          {value || <span className="text-slate-400 italic text-sm font-normal">Not provided</span>}
        </p>
      </div>
      <svg className="w-4 h-4 text-slate-300 group-hover:text-violet-400 flex-shrink-0 ml-3 transition-colors duration-150" viewBox="0 0 20 20" fill="currentColor">
...
// ── Ineligible screen ─────────────────────────────────────────────────────────
function IneligibleScreen({ status, onBack }: { status: string; onBack: () => void }) {
      ...
      >
        <div className="bg-white/80 backdrop-blur-xl border border-black/[0.06] rounded-2xl shadow-xl shadow-black/[0.06] overflow-hidden">
          ...
          <div className="px-7 py-8">
            ...
            <h2 className="text-xl font-bold text-slate-900 mb-3">We're sorry…</h2>
            <p className="text-sm text-slate-500 font-light leading-relaxed mb-6">
              We are working hard to make sure everyone can be covered, but, at the moment,
              you are not eligible for private health insurance as a{" "}
              <span className="font-medium text-slate-700">{status.toLowerCase()}</span>.
              We'd like to help you get another health insurance. As your eligibility for
              different types of insurances depends on quite a few factors, we would suggest
              that you use our{" "}
              <span className="text-violet-600 font-medium underline underline-offset-2 cursor-pointer">
                Recommendation Tool
              </span>{" "}
              to help you figure out which health insurance options are best for you based on
              your situation.
            </p>
...
const updateStep = useApplicationStore((s) => s.updateStep);
  ...
  const questions = [
    { key: "employmentStatus",        title: "What is your employment status?",                          type: "radio",    options: ["Employed", "Self employed", "Civil servant", "University student", "Other"] },
    { key: "jobTitle",                title: "What is your job title?",                                   type: "text",     placeholder: "Job title" },
    { key: "employerName",            title: "What is the name of your employer?",                        type: "text",     placeholder: "Employer name" },
    { key: "startDate",               title: "What is the start date of your employment contract?",        type: "date" },
    { key: "workContract",            title: "Please upload a signed copy of your work contract",          type: "file",     hint: "Since you are employed we require a signed copy of the employment contract from your employer.", provideLater: true },
    { key: "annualIncome",            title: "What is your annual income in Euros before taxes?",          type: "currency", placeholder: "Annual Income" },
    { key: "employedOutsideGermany",  title: "Are you employed outside of Germany?",                       type: "yesno" },
    { key: "hasGermanTaxId",          title: "Do you have a German tax ID?",                               type: "yesno",    info: "taxId" },
    { key: "taxIdAccepted",           title: "Consent & Tax ID Agreement",                                 type: "consent" },
  ] as const;
...
if (!id || Object.keys(form).length === 0) return;
  ...
  const handleFileChange = async (files: FileList | null) => {
    ...
    reader.onload = () => {
      handleChange("workContract", { name: file.name, size: file.size, base64: reader.result });
    };
  ...
  const handleNext = () => {
    ...
    if (stepIndex === 0 && INELIGIBLE.includes(form.employmentStatus)) {
  ...
  if (showIneligible) {
    return (
      <IneligibleScreen
        ...
        onBack={() => { setShowIneligible(false); setStepIndex(0); }}
      />
    );
  }
  const steps = [
...
useEffect(() => {
  ...
  if (showSummary) {
    const summaryRows = [
      { label: "Employment status",       value: form.employmentStatus || "",                                                                                                  step: 0 },
      { label: "Job title",               value: form.jobTitle || "",                                                                                                          step: 1 },
      { label: "Employer name",           value: form.employerName || "",                                                                                                      step: 2 },
      { label: "Employment start date",   value: form.startDay && form.startMonth && form.startYear ? `${form.startYear}-${String(form.startMonth).padStart(2,"0")}-${String(form.startDay).padStart(2,"0")}` : "", step: 3 },
      { label: "Copy of work contract",   value: form.workContract?.name || "Provided separately",                                                                            step: 4 },
      { label: "Income",                  value: form.annualIncome ? `€${Number(form.annualIncome).toLocaleString()}` : "",                                                    step: 5 },
      { label: "Employed outside Germany", value: form.employedOutsideGermany === "Yes" ? "Yes" : form.employedOutsideGermany === "No" ? "No" : "",                           step: 6 },
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50 relative overflow-hidden">
        ...
        <div className="max-w-xl mx-auto px-4 py-8 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }} className="mt-6">
            ...
            <div className="bg-white/80 backdrop-blur-xl border border-black/[0.06] rounded-2xl shadow-sm shadow-black/[0.04] overflow-hidden mb-5">
              {summaryRows.map((row, i) => (
                <motion.div key={row.label} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04, duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
                  <SummaryRow label={row.label} value={row.value} onEdit={() => { setShowSummary(false); setStepIndex(row.step); }} />
                </motion.div>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="flex items-center gap-3 mt-6">
              <button onClick={() => { setShowSummary(false); setStepIndex(totalSteps - 1); }} className="text-slate-400 hover:text-slate-600 transition-colors flex-shrink-0 p-1">
              ...
              <div className="flex-1 h-1 bg-slate-200 rounded-full overflow-hidden"><div className="h-full w-full bg-slate-700 rounded-full" /></div>
              <span className="text-xs text-slate-400 font-medium flex-shrink-0">{totalSteps}/{totalSteps}</span>
            </motion.div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto relative z-10">
        ...
        <div className="mt-6 mb-3">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs text-slate-400">Question {stepIndex + 1} of {totalSteps}</span>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="inline-flex items-center gap-2 bg-violet-500/10 border border-violet-500/20 rounded-full px-3 py-1 mb-4">
              ...
              <span className="text-violet-700 text-[10px] font-semibold tracking-[0.12em] uppercase">Financial Info · {stepIndex + 1}/{totalSteps}</span>
            </div>

            {/* Title */}
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight mb-1">{current.title}</h1>
            {current.hint && <p className="text-sm text-slate-400 font-light mb-5 leading-relaxed">{current.hint}</p>}
            ...
            {current.type === "radio" && (
              <div className="space-y-2">
                {current.options?.map((opt: string) => {
                  ...
                  return (
                    <motion.button key={opt} onClick={() => handleChange(current.key, opt)} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
            ...
            {current.type === "date" && (
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { field: "startMonth", label: "Month", placeholder: "MM" },
                  { field: "startYear",  label: "Year",  placeholder: "YYYY" },
            ...
            {current.type === "file" && (
              <div>
                <motion.div
                  ...
                  animate={dragOver ? { borderColor: "rgba(139,92,246,0.6)", backgroundColor: "rgba(139,92,246,0.04)", scale: 1.01 } : { borderColor: "rgba(0,0,0,0.08)", backgroundColor: "rgba(248,250,252,1)", scale: 1 }}
                  transition={{ duration: 0.2 }}
                  className="border-2 border-dashed rounded-xl p-8 flex flex-col items-center gap-3 cursor-pointer text-center"
                >
                  <motion.div animate={dragOver ? { scale: 1.15, y: -4 } : { scale: 1, y: 0 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}
                  ...
                  {form.workContract ? (
                    <div>
                      <p className="text-sm font-semibold text-violet-700">{form.workContract.name}</p>
                      <p className="text-xs text-slate-400 mt-0.5">{(form.workContract.size / 1024).toFixed(1)} KB · Click to replace</p>
                    </div>
                  ) : (
                    <div>
                      <p className="text-sm font-medium text-slate-600">{dragOver ? "Drop file here" : "Choose file or drag & drop"}</p>
                      <p className="text-xs text-slate-400 mt-0.5">Supports images, videos and documents</p>
                    </div>
                  )}
                </motion.div>
                <input ref={fileInputRef} type="file" className="hidden" onChange={(e) => handleFileChange(e.target.files)} />
              </div>
            )}
            ...
            {current.type === "yesno" && (
              <div className="space-y-2">
                ...
                {current.info === "taxId" && (
                  <div className="mt-2">
                    <button onClick={() => setShowTaxInfo((p) => !p)} className="text-xs text-violet-600 underline underline-offset-2 hover:text-violet-700">
                      ...
                      {showTaxInfo && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                          <div className="mt-3 bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-500 leading-relaxed space-y-2">
                            <p><strong className="text-slate-700">Tax ID (Steuer-ID):</strong> A unique number automatically assigned to you when you first register in Germany. It is not the same as your tax number (Steuernummer).</p>
                            <p className="font-medium text-slate-600">Why do we need your tax ID?</p>
                            <ul className="space-y-1.5 list-disc list-inside">
                              <li>The insurer requires your tax ID to transmit data to the tax authorities. As of 01.01.26, insurers are legally obligated to electronically transmit employer-subsidized contributions to the Federal Central Tax Office.</li>
                              <li>The insurer also reports tax-deductible contributions to health and long-term care insurance to the tax authorities for your income tax assessment.</li>
                              <li>If you do not provide your tax ID, the insurer is entitled to request it from the Federal Central Tax Office.</li>
                            </ul>
                          </div>
                        </motion.div>
                      )}
            ...
            {current.type === "consent" && (
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                  <p className="text-xs font-semibold text-slate-700 mb-2">Consent to submit Schufa</p>
                  <p className="text-xs text-slate-500 font-light leading-relaxed mb-3">
                    By selecting "I accept", I consent to the exchange of data with Schufa in accordance with the{" "}
                    <span className="text-violet-600 underline underline-offset-2 cursor-pointer">Schufa information</span>.
                  </p>

                  <motion.button
                    onClick={() => handleChange("taxIdAccepted", !form.taxIdAccepted)}
                  ...
                  >
                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all duration-150 ${form.taxIdAccepted ? "border-violet-600 bg-violet-600" : "border-slate-300"}`}>
                      {form.taxIdAccepted && (
                    ...
                    <span className={`text-sm font-medium ${form.taxIdAccepted ? "text-violet-800" : "text-slate-600"}`}>I accept the terms above</span>
                  </motion.button>
                </div>

                {/* What is Schufa toggle */}
                <div>
                  <button onClick={() => setShowSchufaInfo((p) => !p)} className="text-xs text-violet-600 underline underline-offset-2 hover:text-violet-700">
                    ...
                    {showSchufaInfo && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                        <div className="mt-3 bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-500 leading-relaxed">
                          <p><strong className="text-slate-700">Schufa</strong> is a credit bureau. Based on data from banks and other institutions, it assesses your creditworthiness.</p>
                          <p className="mt-2"><strong className="text-slate-600">Why it is used for your health insurance:</strong> The risk carrier needs to check whether the monthly costs of the health insurance will be covered by their customers, so they can offer a safe and stable product.</p>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
            ...
            <div className="mt-6 flex gap-3">
              {stepIndex > 0 && (
                <motion.button onClick={() => { setStepIndex((p) => p - 1); setError(null); }} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
            ...
            {current.provideLater && (
              <button onClick={handleProvideLater} className="w-full mt-2 text-xs text-slate-400 hover:text-slate-600 transition-colors py-1">
                Provide later →
              </button>
            )}
          </motion.div>
